...
export default function Projects({ portfolios }: { portfolios: Portfolio[] }) {
  ...
  return (
    <section className="relative bg-transparent mb-8 p-16 border-y border-black flex flex-col md:flex-row">
        ...
        <div className="md:col-span-1 md:flex md:items-center md:justify-center">
          <ul className="space-y-4">
            {portfolios.map((portfolio: Portfolio, index: number) => {
              const hoverStyle = index === hoverIndex ? 'border border-black' : 'opacity-50 border border-transparent';
              return (
                <li
                  key={index}
                  className={`flex items-center space-x-4 cursor-pointer p-4 box-border ${hoverStyle}`}
